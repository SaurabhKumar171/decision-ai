...
export default function Home() {
  ...
  const handlePreset = (presetSkills: string, presetGoal: string, presetConstraints: string) => {
  ...
  return (
    <div className="relative flex w-full min-h-[calc(100vh-4rem)] overflow-hidden flex-col lg:flex-row">
                  ...
                  >
                    {isLoading || isFetchingResources ? (
                      ...
                      >
                        ...
                        <span className="animate-pulse">{isLoading ? "Synthesizing Optimal Path..." : "Curating Resources..."}</span>
                      </motion.div>
                    ) : (
            ...
            >
              <div className="text-center space-y-6 flex flex-col items-center">
                ...
                <p className="text-muted-foreground text-lg max-w-lg">
                  Input your dilemma on the left, or pick a pre-loaded scenario below to test instantly.
                </p>
              </div>
              ...
              <div className="w-full space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={() => handlePreset("React/Node.js, 4 YOE", "FAANG AI Engineer", "10 hrs/week, $500 budget for courses")}
                  ...
                  >
                    ...
                    <div className="text-xs text-muted-foreground line-clamp-2">Skills: React/Node.js • Goal: FAANG AI Eng • Constraints: 10 hrs/wk, $500</div>
                  </motion.button>

                  <motion.button
                    whileHover={{ scale: 1.03, y: -2 }}
                    whileTap={{ scale: 0.98 }}
                    onClick={() => handlePreset("Python/Django", "Launch SaaS in 30 days", "Solo dev, $0 infra budget")}
                  ...
                  >
                    ...
                    <div className="text-xs text-muted-foreground line-clamp-2">Skills: Python • Goal: Launch in 30d • Constraints: Solo, $0 budget</div>
                  </motion.button>

                  <motion.button
                    whileHover={{ scale: 1.03, y: -2 }}
                    whileTap={{ scale: 0.98 }}
                    onClick={() => handlePreset("SQL/Excel, no coding", "Machine Learning Engineer", "No math background, self-paced learning")}
                  ...
                  >
                    ...
                    <div className="text-xs text-muted-foreground line-clamp-2">Skills: SQL/Excel • Goal: ML Engineer • Constraints: No math bg</div>
                  </motion.button>
                </div>
              </div>
              ...
              <div className="w-full pt-12 mt-12 border-t border-border/40 flex justify-center">
                <div className="flex items-center gap-3 md:gap-6 text-xs md:text-sm font-medium text-muted-foreground opacity-60">
                  <div className="flex flex-col items-center gap-2">
                    <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center"><SlidersHorizontal className="w-4 h-4" /></div>
                    <span>Context Input</span>
                  </div>
                  <div className="h-[2px] w-8 md:w-16 bg-gradient-to-r from-muted to-primary/30"></div>
                  <div className="flex flex-col items-center gap-2">
                    <div className="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center"><Brain className="w-4 h-4" /></div>
                    <span>Gemini Engine</span>
                  </div>
                  <div className="h-[2px] w-8 md:w-16 bg-gradient-to-r from-primary/30 to-blue-500/30"></div>
                  <div className="flex flex-col items-center gap-2">
                    <div className="w-8 h-8 rounded-full bg-blue-500/10 text-blue-500 flex items-center justify-center"><Network className="w-4 h-4" /></div>
              ...
              <div className="flex items-center justify-between pb-2">
                ...
                {decision && (
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" onClick={copyToClipboard} className="h-9 gap-1.5 font-medium">
                      {hasCopied ? <CheckCircle2 className="w-3.5 h-3.5 text-green-500" /> : <Copy className="w-3.5 h-3.5" />}
                      {hasCopied ? "Copied!" : "Copy Summary"}
                    </Button>
                    <Button variant="ghost" size="sm" onClick={handleReset} className="h-9 gap-1.5 font-medium text-muted-foreground hover:text-foreground">
                          ...
                          <ul className="space-y-3">
                            {decision.tradeoffsAccepted.map((tradeoff, index) => (
                              ...
                              >
                                ...
                                <span className="text-sm leading-snug">{tradeoff}</span>
                              </motion.li>
                            ))}
                          </ul>
                        </div>
              ...
              {isFetchingResources && (
                ...
                >
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    {[1, 2, 3, 4].map((i) => (
                      <div key={i} className="h-40 bg-muted/30 rounded-xl border border-border/30 animate-pulse"></div>
                    ))}
                  </div>
                </motion.div>
              )}
